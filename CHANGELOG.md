...
- Added Phase 3.3 service-container foundation with explicit Core service registration, an idempotent freeze boundary, PSR-11 read-only `has()`/`get()` interoperability, application and transient lifetimes, reserved-but-rejected Execution lifetime vocabulary, deterministic unknown-service, circular-dependency and factory-failure behavior, and `ApplicationKernel` freeze-before-readiness integration; autowiring, aliases, service tags, decorators, service-locator globals, execution scopes, reset handling, HTTP, module/plugin runtime, console and telemetry remain deferred.

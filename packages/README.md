...
| `evolvephp/http` | `Evolve\Http\` | HTTP boundary with the Phase 4.1 PSR HTTP interoperability and `MiddlewarePipeline` foundation, Phase 4.2 route definitions and matching, Phase 4.3 routed handler dispatch with typed routing failures, and Phase 4.4 `HttpKernel` integration with Core execution orchestration; response/error rendering and runtime adapters remain deferred. |

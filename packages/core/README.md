...
`ExecutionContext` carries the generated execution identifier, `ExecutionKind` and immutable `ExecutionContextValues`. Context values are optional per-execution locale and timezone strings supplied after application policy has resolved them; absent values remain `null`, are not inferred from PHP process defaults and do not modify PHP's default locale or timezone. Core does not provide translation catalogs, formatting, locale negotiation or fallback policy, but future application localization code can read the explicit context values without relying on ambient current-execution state.

Core also provides optional generic execution-lifecycle observation hooks around the currently implemented orchestration boundary. A caller may pass no observation sink, one `Evolve\Core\Instrumentation\ObservationSink` or an explicitly ordered list of sinks to receive immutable safe observations for execution start, handler completion, scope close, quarantine requirement and execution completion. Observation failure is isolated: sink throwables are translated into safe `InstrumentationFailure` values on `ExecutionOutcome`, later sinks still receive the same observation, every sink failure is retained in configuration order and observation failures do not replace the primary handler result or throwable, do not replace cleanup failure and do not alter the reusable/quarantine decision.

`Evolve\Core\Execution\ExecutionContextAttacher` and `ExecutionContextAttachment` provide a generic lifecycle boundary for caller-owned external execution context. Core invokes configured attachers in declaration order after creating the immutable `ExecutionContext` and before emitting the execution-started observation. Successful attachments are retained only for that execution and detach in reverse successful-attachment order after scope-close-started observation and before `ExecutionScope::close()`. Attachment establishment failures are safe instrumentation failures associated with execution start and do not require quarantine; attachers that throw while establishing an attachment are responsible for leaving no active attachment behind. Attachment detach failures are execution-isolation cleanup failures, are aggregated through `ExecutionCleanupFailed`, continue best-effort cleanup, preserve the primary handler result or throwable and require quarantine.
...
The generic instrumentation foundation is deliberately not Evolve Insight, Evolve Observe or OpenTelemetry. Core does not provide tracing, metrics, logs, spans, propagation, storage, exporters, asynchronous telemetry processing, HTTP instrumentation, module/plugin instrumentation, Bridge instrumentation or runtime-adapter instrumentation.
